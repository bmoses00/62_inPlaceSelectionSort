...
public class OrderedList_inArraySlots
    ...
    public OrderedList_inArraySlots (ArrayList<Integer> unordered) {
        for (int counter = 0; counter < unordered.size(); counter++) {
            int index = champIndex(unordered, counter);
            int lowest = unordered.get(index);
            unordered.set(index, unordered.get(counter));
            unordered.set(counter, lowest);
        }
        list_iAS = unordered; // is now ordered
    }

    /**
      helper function for constructor
      Write good English here, reflecting good thinking
      @return index of lowest el
     */

    private int champIndex(ArrayList<Integer> challengers, int startLocation) {
